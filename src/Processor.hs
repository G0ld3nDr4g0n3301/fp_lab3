...
generateXs start end genStep isLastSegment =
    ...
    in
        takeWhile (< limit) [start, start + genStep ..]
...
runSlidingWindow config inputStream =
    ...
    in
        ...
        then []
            ...
            in
                firstChunk ++ loop mid initialWindow restStream
    where
        ...
        loop mid window [] =
            ...
            in
                calcSegment config window start end True
        
        loop mid window (newPoint:rest) =
            let
                newWindow = drop 1 window ++ [newPoint]
                start = fst (window !! mid)
                end   = fst (newWindow !! mid)
            in
                calcSegment config newWindow start end False ++ loop mid newWindow rest

        calcSegment :: AlgoConfig -> [Point] -> Double -> Double -> Bool -> [(String, Point)]
        calcSegment conf window startX endX isLast =
            let
                xs = generateXs startX endX (step conf) isLast
