...
newtonInterpolate points x =
  let coefs = calcNewtonCoeffs points
      xs = map fst points
   in evalNewton xs coefs x
  ...
  let (xs, ys) = unzip points
      ...
      calcLayers step currentYs =
        let yPairs = zip currentYs (drop 1 currentYs)
            ...
            nextYs =
              map
                ( \((y_curr, y_next), (x_curr, x_next)) ->
                    (y_next - y_curr) / (x_next - x_curr)
                )
                pairs
         in nextYs : calcLayers (step + 1) nextYs
      ...
      getCoeffs (y : _) layers = y : map safeHead layers
        where
          ...
          safeHead (h : _) = h
      getCoeffs _ _ = []
   in getCoeffs ys (calcLayers 1 ys)
